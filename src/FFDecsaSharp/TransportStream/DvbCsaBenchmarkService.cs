...
/// <summary>Describes one deterministic DVB-CSA benchmark run.</summary>
public readonly record struct DvbCsaBenchmarkResult(double BytesPerSecond, double ElapsedSeconds, long PacketsProcessed);

/// <summary>Runs the same packet decryption benchmark used by the desktop application.</summary>
public static class DvbCsaBenchmarkService
{
    /// <summary>The number of packets in one bitsliced SIMD batch.</summary>
    public const int BatchSize = 128;
    /// <summary>The fixed warmup workload, expressed in bitsliced batches.</summary>
    public const int WarmupBatches = 2_000;
    /// <summary>The default measured workload, expressed in bitsliced batches.</summary>
    ...
    /// <summary>Gets the supported, stable measurement workloads.</summary>
    public static IReadOnlyList<int> MeasurementBatchOptionValues => MeasurementBatchOptions;

    /// <summary>Runs the deterministic benchmark asynchronously.</summary>
    public static Task<DvbCsaBenchmarkResult> RunAsync(int workerCount, int measurementBatches, CancellationToken cancellationToken = default) =>
        Task.Run(() => Run(workerCount, measurementBatches, cancellationToken), cancellationToken);

    /// <summary>Returns the supported workload closest to the requested value.</summary>
    ...
    private static DvbCsaBenchmarkResult Run(int workerCount, int measurementBatches, CancellationToken cancellationToken)
    {
        ...
        int effectiveWorkerCount = TransportStreamDecryptionService.CoerceWorkerCount(workerCount);
        int packetCountPerBlock = TransportStreamDecryptionService.GetPacketsPerBlock(effectiveWorkerCount);
        ...
        var source = new byte[TransportPacket.Size * packetCountPerBlock];
        ...
        {
            Span<byte> packet = source.AsSpan(packetIndex * TransportPacket.Size, TransportPacket.Size);
        ...
        return new DvbCsaBenchmarkResult(packetCount * (double)PayloadSize / elapsedSeconds, elapsedSeconds, packetCount);
    }
